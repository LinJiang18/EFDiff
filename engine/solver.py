...
class Trainer(object):
    def __init__(self, config, args, model, dataloader, logger=None, ext_trainer: Optional[object] = None):
        super().__init__()
        self.model = model
        ...
        self.results_folder = Path(config['solver']['results_folder'] + f'_{model.seq_length}_topk({model.top_k_frequency})')
    ...
    def save_classifier(self, milestone, verbose=False):
        if self.logger is not None and verbose:
            self.logger.log_info(
                'Save current classifer to {}'.format(str(self.results_folder / f'ckpt_classfier-{milestone}.pt')))
        data = {
