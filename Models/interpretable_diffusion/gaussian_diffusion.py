...
class Diffusion_TS(nn.Module):
    def __init__(
        ...
        self.model = Transformer(n_feat=feature_size, n_channel=seq_length, top_k_frequency= top_k_frequency,  n_layer_enc=n_layer_enc,
    ...
    def output(self, x, t, pred_bin,padding_masks=None):
        trend, season = self.model(x, t, padding_masks=padding_masks, ext_bins=pred_bin)  # decompose transformer
    ...
    def get_ext_bins(self, x_t: torch.Tensor, t: torch.Tensor, k: Optional[int] = None) -> torch.Tensor:
        """
        调用注入的 ext_trainer 预测 top-k 频率 bin。
        x_t: (B, L, 1), t: (B,)
        返回 (B, k) LongTensor（在当前模型设备上）。
        """
    ...
    def _train_loss(self, x_start, t, target=None, noise=None, padding_masks=None):
        ...
        # 1) 得到加噪样本 x_t
        x = self.q_sample(x_start=x_start, t=t, noise=noise)  # (B, L, C)

        # 2) 调用 ext_trainer 预测 top-k 频率的 bin
        pred_bins = None
        if getattr(self, "ext_trainer", None) is not None:
            # 确保形状/类型满足 ext_trainer 期望
            # ext_trainer 期望最后一维是 1 通道；若你的 feature_size>1，这里取第一个通道
            if x.size(-1) != 1:
                x_for_ext = x[..., :1].contiguous()  # (B, L, 1)
            else:
                x_for_ext = x

            # t 应为 (B,) 的 long tensor；若是标量或形状不匹配，这里对齐
            if t.dim() == 0:
            ...
            # ★ 关键调用：使用你在类里实现的 get_ext_bins 封装
            with torch.no_grad():
                pred_bins = self.get_ext_bins(x_for_ext, t, k=k)  # (B, k) LongTensor（在当前 model 设备上）

            self._last_ext_bins = pred_bins  # 可选：供外部/日志调试

        model_out = self.output(x, t, pred_bins,padding_masks)  # 你的实现里这代表 x0
