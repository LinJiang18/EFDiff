...
class EFDiff(nn.Module):
    def __init__(
        ...
        use_ff=True,
        reg_weight=None,
        ext_trainer: Optional[object] = None,
        **kwargs
    ):
        super(EFDiff, self).__init__()

        self.eta, self.use_ff = eta, use_ff
        self.seq_length = seq_length
        self.feature_size = feature_size
        self.top_k_frequency = top_k_frequency
    ...
    def set_ext_trainer(self, ext_trainer):
        self.ext_trainer = ext_trainer
    ...
    def output(self, x, t, pred_bin, padding_masks=None):
        trend, season = self.model(x, t, padding_masks=padding_masks, ext_bins=pred_bin)
    ...
    def p_sample(self, x, t: int, clip_denoised=True, cond_fn=None, model_kwargs=None):
        ...
        if cond_fn is not None:
            model_mean = self.condition_mean(cond_fn, model_mean, model_log_variance, x, t=batched_times, model_kwargs=model_kwargs)
    ...
    @torch.no_grad()
    def get_ext_bins(self, x_t: torch.Tensor, t: torch.Tensor, k: Optional[int] = None) -> torch.Tensor:
        if self.ext_trainer is None:
            raise RuntimeError("ext_trainer is not set on Diffusion_TS")
        et = self.ext_trainer
        bins_ll = et.predict_bins(x_t.to(et.cfg.device), t.to(et.cfg.device), k=k)
        return torch.as_tensor(bins_ll, dtype=torch.long, device=x_t.device)
    ...
    def _train_loss(self, x_start, t, target=None, noise=None, padding_masks=None):
        ...
        pred_bins = None
        if getattr(self, "ext_trainer", None) is not None:
            if x.size(-1) != 1:
                x_for_ext = x[..., :1].contiguous()
            else:
                x_for_ext = x
            if t.dim() == 0:
                t = t.view(1).expand(x_for_ext.size(0))
            t = t.to(dtype=torch.long, device=x_for_ext.device)
            k = int(getattr(self, "top_k_frequency", 5))
            with torch.no_grad():
                pred_bins = self.get_ext_bins(x_for_ext, t, k=k)
            self._last_ext_bins = pred_bins
        model_out = self.output(x, t, pred_bins, padding_masks)
    ...
    def fast_sample_infill(self, shape, target, sampling_timesteps, partial_mask=None, clip_denoised=True, model_kwargs=None):
        ...
        for time, time_next in tqdm(time_pairs, desc='conditional sampling loop time step'):
            ...
            img = self.langevin_fn(sample=img, mean=pred_mean, sigma=sigma, t=time_cond, tgt_embs=target, partial_mask=partial_mask, **model_kwargs)
    ...
    def sample_infill(self, shape, target, partial_mask=None, clip_denoised=True, model_kwargs=None):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='conditional sampling loop time step', total=self.num_timesteps):
            img = self.p_sample_infill(x=img, t=t, clip_denoised=clip_denoised, target=target, partial_mask=partial_mask, model_kwargs=model_kwargs)
        img[partial_mask] = target[partial_mask]
        return img
    ...
    def sample_cond(self, shape, clip_denoised=True, model_kwargs=None, cond_fn=None):
        ...
        img = torch.randn(shape, device=device)
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img, x_start = self.p_sample(img, t, clip_denoised=clip_denoised, cond_fn=cond_fn, model_kwargs=model_kwargs)
        return img

    def fast_sample_cond(self, shape, clip_denoised=True, model_kwargs=None, cond_fn=None):
        batch, device, total_timesteps, sampling_timesteps, eta = shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.eta
        times = torch.linspace(-1, total_timesteps - 1, steps=sampling_timesteps + 1)
        times = list(reversed(times.int().tolist()))
        time_pairs = list(zip(times[:-1], times[1:]))
