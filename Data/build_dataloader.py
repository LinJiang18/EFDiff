...
def build_dataloader_cond(config, args=None, samples = None):
    ...
    elif args.mode == 'longpredict':
        config['dataloader']['long_predict_dataset']['params']['output_dir'] = args.save_dir
        config['dataloader']['long_predict_dataset']['params']['predict_length'] = args.pred_len
        config['dataloader']['long_predict_dataset']['params']['predict_step'] = args.pred_step
    else:
        raise ValueError(f"Unsupported mode: {args.mode}. Please choose from ['infill', 'predict', 'longpredict'].")
    if samples is not None:
        if args.mode == 'longpredict':
            config['dataloader']['long_predict_dataset']['params']['rawdata'] = samples
            test_dataset = instantiate_from_config(config['dataloader']['long_predict_dataset'])
        else:
            config['dataloader']['test_dataset']['params']['rawdata'] = samples
